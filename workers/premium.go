...
func assemble() string {
	var gravityInstalled = current("bcgov-plugin/gravityforms")
	var ticketsInstalled = current("bcgov-plugin/event-tickets-plus")
	var polylangInstalled = current("bcgov-plugin/polylang-pro")
	var gravityAvailable = latest("https://docs.gravityforms.com/gravityforms-change-log/", "<h3>")
	...
	collect := results(ticketsAvailable, ticketsInstalled, "event-tickets-plus") + results(polylangAvailable, polylangInstalled, "polylang-pro") + results(gravityAvailable, gravityInstalled, "gravityforms")
	return collect
}
